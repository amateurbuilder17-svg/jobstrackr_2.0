...
/**
 * Every indexable job in one sector or at one qualification level, or every
 * update of one kind (results, admit cards, answer keys…).
 *
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return hubRouteMetadata("category", slug);
}

export default async function CategoryHubPage({ params }: { params: Params }) {
  const { slug } = await params;
  return renderHubRoute("category", slug);
}
