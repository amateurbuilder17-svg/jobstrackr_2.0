...
export function SectionHeaderSkeleton({
  ...
  return (
    <div className="mb-3.5 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3" aria-hidden="true">
      <div className="flex min-w-0 gap-3">
        <span className="mt-0.5 w-[3px] shrink-0 rounded-full bg-brand/40" />
        <div className="min-w-0 space-y-1.5">
          <div className={cn("skeleton h-5", titleWidth)} />
          {hasSubtitle ? <div className="skeleton h-3.5 w-48" /> : null}
        </div>
      </div>
    ...
    <section aria-label="Loading your exams" className="animate-in fade-in duration-200">
      ...
      <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-1 no-scrollbar scroll-pl-4 scroll-px-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div
            key={i}
            className="w-[288px] shrink-0 rounded-2xl border border-border bg-card p-3.5 shadow-card"
          >
...
export function ClosingSoonSkeleton() {
  return (
    <section aria-label="Loading closing soon recruitments" className="animate-in fade-in duration-200">
      <SectionHeaderSkeleton titleWidth="w-28" />
      <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-1 no-scrollbar scroll-pl-4 scroll-px-4">
        {Array.from({ length: 2 }).map((_, i) => (
          <div
            key={i}
            className="w-[320px] shrink-0 rounded-2xl border border-brand/25 bg-brand-soft/70 p-3.5 shadow-card"
          >
...
export function JustPublishedSkeleton() {
  return (
    <section aria-label="Loading just published recruitments" className="animate-in fade-in duration-200">
      <SectionHeaderSkeleton titleWidth="w-28" />
        ...
        {Array.from({ length: 4 }).map((_, i) => (
          ...
          >
            <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-3">
              ...
              <div className="skeleton size-4 rounded-xs shrink-0 mt-1" />
            </div>
          </div>
        ))}
      </div>
    ...
    <section aria-label="Loading popular exams" className="animate-in fade-in duration-200">
      ...
      <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-1 no-scrollbar scroll-pl-4 scroll-px-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="flex w-[140px] shrink-0 flex-col gap-2 rounded-2xl border border-border bg-card p-3.5 shadow-card"
          >
            <div className="skeleton size-9 rounded-xl" />
