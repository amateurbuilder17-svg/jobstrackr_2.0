...
describe("userHasPassword", () => {
  ...
  it("is false for a Google account that has never set one", () => {
    ...
    expect(userHasPassword(userWith({ provider: "google", providers: ["google"] }))).toBe(false);
  });
  ...
  it("is true once a password has been set on a Google account", () => {
    ...
    expect(
      userHasPassword(userWith({ provider: "google", providers: ["google"] }, { has_password: true })),
    ).toBe(true);
  });

  it("treats a linked account with both methods as having one", () => {
    expect(userHasPassword(userWith({ provider: "email", providers: ["email", "google"] }))).toBe(
      true,
    );
  });
  ...
  it("does not accept a truthy non-boolean as proof", () => {
    ...
    expect(userHasPassword(userWith({ providers: ["google"] }, { has_password: "no" }))).toBe(false);
    expect(userHasPassword(userWith({ providers: ["google"] }, { has_password: 1 }))).toBe(false);
  });
